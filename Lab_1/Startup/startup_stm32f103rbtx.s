...
data_cmp:
	...
	cmp		r4, r5
	beq		no_change           // если DT не изменился — ничего не делаем

// проверяем, что CLK = 1 (фронт CLK)
	mov     r3, r0				// создание копии значений из r0 в r3
	lsr     r3, #8				// сдвиг вправо на 8 бит для получения тек значения clk
	and     r6, r3, #1          // оставляем только младший бит по маске
	cmp     r6, #1				// реагируем только на момент, когда CLK = 1
	bne     no_change

	b       dt_changed          // теперь переход только при фронте CLK
