...
// инициализация порта С
    ...
    mov		r8, #8						// запись в регистр r8 начального значения для вывода на семисегментник
...
case_0:
	...
	ite		eq
    ldreq 	r9, =0x3F           // значения сегментов для "0"
    ldrne 	r9, =0xBF
...
turn_right:
	...
	movlo	r1, #0		// то: "выставить 8 бит (точку) в 0 (лог И по маске 0111_1111)
	movhs	r1, #1		// иначе: "выставить 8 бит (точку) в 1 (лог ИЛИ по маске 1000_0000)"
...
turn_left:
	...
	movhi	r1, #0		// то: "выставить 8 бит (точку) в 0 (лог И по маске 0111_1111)
	orrls	r1, #1		// иначе: "выставить 8 бит (точку) в 1 (лог ИЛИ по маске 1000_0000)"
